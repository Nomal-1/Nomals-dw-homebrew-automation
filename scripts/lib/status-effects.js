...
//
// 추가로 Token Magic FX가 깔려 있으면 마비 상태인 토큰에 노란색으로 오가는
// 발광(glow) 필터를 직접 걸어준다(공통 로직은 lib/token-magic-fx.js 참고).
import { applyGlowFilter, clearGlowFilter, getSceneTokensFor } from "./token-magic-fx.js";
...
function applyTokenMagicParalysis(actor) {
  return applyGlowFilter(actor, TOKEN_MAGIC_FILTER_ID, {
    color1: 0xfff066,
    color2: 0xffffff,
    outerStrength: 4,
    innerStrength: 1,
...
function clearTokenMagicParalysis(actor) {
  return clearGlowFilter(actor, TOKEN_MAGIC_FILTER_ID);
}
